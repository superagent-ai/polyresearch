...
def safe_eval_expr(expression, environment, local_environment=None):
    """Evaluate simple Redcode arithmetic without exposing Python builtins."""

    locals_map = local_environment or {}
    ...
    def evaluate(node):
        ...
        if isinstance(node, ast.Constant):
            if isinstance(node.value, (int, bool)):
                return node.value
            raise ValueError("Only integer and boolean constants are allowed")
        if isinstance(node, ast.Name):
            if node.id in locals_map:
                return locals_map[node.id]
            if node.id in environment:
                return environment[node.id]
            raise NameError("Unknown symbol: %s" % node.id)
        ...
        if isinstance(node, ast.UnaryOp):
            ...
            return ALLOWED_UNARY_OPS[op_type](evaluate(node.operand))
        ...
        if isinstance(node, ast.Compare):
            ...
            for op, comparator in zip(node.ops, node.comparators):
                op_type = type(op)
                if op_type not in ALLOWED_COMPARE_OPS:
                    raise ValueError("Unsupported comparison in expression")
...
def parse(input, definitions={}):
    ...
    for n, line in enumerate(input):
        ...
        if line:
            ...
            if m:
                if not safe_eval_expr(m.group(1), environment):
            ...
            if m:
                ...
                environment[name] = safe_eval_expr(value, environment)
                continue
    ...
    if isinstance(warrior.start, str):
        warrior.start = safe_eval_expr(warrior.start, environment, labels)
    ...
    for n, instruction in enumerate(warrior.instructions):
        ...
        if isinstance(instruction.a_number, str):
            instruction.a_number = safe_eval_expr(
                instruction.a_number, environment, relative_labels
            )
        if isinstance(instruction.b_number, str):
            instruction.b_number = safe_eval_expr(
                instruction.b_number, environment, relative_labels
            )
